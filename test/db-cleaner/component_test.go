...
func TestCleanerRemovesExpiredRowsFromPostgreSQL(t *testing.T) {
	...
	setupCleaner(t, ctx, net,
		map[string]string{
			...
			"DATABASE_URL":                 "postgres://postgresql.testcontainer.docker:5432/database?sslmode=disable",
...
func TestCleanerRemovesExpiredRowsFromSQLServer(t *testing.T) {
	...
	setupCleaner(t, ctx, net,
		map[string]string{
			...
			"DATA_CATALOG_TOKEN":           "eyJraWQiOiJHYj",
			"DATA_CATALOG_DATABASE_SCHEMA": "service.database.schema",
			"DATABASE_URL":                 "sqlserver://mssql.testcontainer.docker:1433?database=database&encrypt=disable",
			"DATABASE_USERNAME":            "sa",
			"DATABASE_PASSWORD":            "SuperStrong@Passw0rd",
			"DATABASE_SCHEMA":              "schema",
		})
